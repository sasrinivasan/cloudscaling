...
def CheckRestError(status_code=500,response=""):
    ...
    if status_code not in okRespCodeList:
        logger.error("Unexpected response from REST server- Response  is %s",response)
        
        logger.error("Calling Function is %s",callerfunction)
        respOK=False
    return respOK
...
def generateOnboardCsvWithAP(apiurl, authurl, xiquser, xiqpass,device_type, num_devices, model="x435-24s", serial_prefix="DEV", serial_start=0, output_file="generated_devices_with_ap.csv", num_aps=0, ap_start_serial="04101900460420", ap_model="AP410C", ap_per_switch=5,auth_token="None"):
    """
    ...
    with open(output_file, mode="w", newline="", encoding="utf-8") as csvfile:
        fieldnames = [
            "SerialNumber", "IPAddress", "DeviceFamily", "User", "Password",
            "InletConnect", "SSHConnect", "InletServerIP", "InletServerPort",
            "Model", "Location"
        ]
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()

        # Generate entries for the main devices
        locations = []  # Store locations for mapping APs
        for i in range(num_devices):
            serial_number = f"{serial_prefix}-{serial_start + i:06d}"
            ip_address = f"192.168.0.{(i % 254) + 1}"  # Ensure IP addresses are within a valid range
            location = floor_locations[i % len(floor_locations)]  # Use the same location logic as generateOnboardCsv
            locations.append(location)  # Store location for mapping APs
            writer.writerow({
                "SerialNumber": serial_number,
                "IPAddress": ip_address,
                "DeviceFamily": device_type,
        ...
        # Generate entries for the APs if num_aps > 0
        if num_aps > 0:
            ap_start_serial = int(ap_start_serial)  # Convert the starting serial to an integer
            ap_count = 0  # Track the number of APs generated
            for i, location in enumerate(locations):
                for j in range(ap_per_switch):
                    if ap_count >= num_aps:
                        break  # Stop if the total number of APs is reached
                    ap_serial_number = f"{ap_start_serial + ap_count:014d}"  # Ensure 14-digit format
                    ap_ip_address = "192.168.1.1"  # Use a dummy IP address for all APs
                    writer.writerow({
                        "SerialNumber": ap_serial_number,
                        "IPAddress": ap_ip_address,
                        "DeviceFamily": "extreme",
                        "User": "admin",
                        "Password": "Extreme@123",
                        "InletConnect": "false",
                        "SSHConnect": "false",
                        ...
                        "InletServerPort": "8090",
                        "Model": ap_model,
                        "Location": location  # Map AP to the same location as the switch
                    })
                    ap_count += 1
...
if __name__=="__main__":
    ...
    xiquser="sasrinivasan+user1@extremenetworks.com"
    xiqpass="Extreme@123"
    ...
    #auth="eyJraWQiOiIyNzNkZjE0MmExMDE0OWYyYmQyYmNmZWMwZTMyMjVhNCIsImFsZyI6IlJTMjU2In0..QzCLI-cizQj0vptfUBiIYxUZldM6LEOwnH_lLZPr1adNKgQp9gmN-ekXdVJRFwzIqsm9dpri6EELO3rbEVQ2brIwHVqHDDlEwnpzA04vAAsYaTKb31XwUtlwPEAG1Tm3km82ZJnuEB8XmCyqs4b8mbI8-A_bvw78jUTa9s-PgauRv4WjEgJdKwQ43zPbhH4QuGaS2iwaiX51a4F5W3d9Yzxzbd2o9WWX9Ypky2_LMkGhirjxAAP4Du4Xgsb3RzTnGpQvmJ2Xp3-bsdyW9OvmM5T8pIZHFizBUKNdyyI58VTL2u5Se_FNzFY6TloykbA1j218gxK1svKaoZa_hJ6SRg"
    #print(auth)
    ...
    #sns=["NIR-000000","NIR-000001"]
    #post_xiqOnboardDevices (apiurl=apiurl,authurl=authurl,xiquser=xiquser,xiqpass=xiqpass,path="devices/:onboard",deviceType="EXOS",field="sns",snsList=sns,auth_token=auth)
